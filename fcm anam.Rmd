...
res.fcm <- fcm(data, centers=3, memberships=u0)
```
...
```{r}
res.fcm <- fcm(data, centers = 3, alginitv = "hartiganwong", alginitu = "imembrand") 
```
...
```{r}
res.fcm <- fcm(data, centers=3, dmetric="correlation")
```
...
```{r}
res.fcm <- fcm(data, centers = 3, m = 4)
```
...
#### 4. VALIDASI HASIL CLUSTERING

Validasi cluster merupakan proses evaluasi atas kebaikan hasil clustering. Untuk tujuan ini, berbagai indeks validitas telah diusulkan dalam literatur terkait. Karena clustering adalah analisis pembelajaran tanpa pengawasan yang tidak menggunakan informasi eksternal, indeks internal digunakan untuk memvalidasi hasil clustering. Meskipun ada banyak indeks internal yang awalnya diusulkan untuk bekerja dengan gelar keanggotaan keras yang dihasilkan oleh K-means dan variannya, sebagian besar indeks ini tidak dapat digunakan untuk hasil pengelompokan fuzzy. Di lingkungan R, Partition Entropy (PE), Partition Coefficient (PC) dan Modified Partition Coefficient (MPC) dan Fuzzy Silhouette Index tersedia dalam paket 'fclust' (Ferraro & Giordani, 2015), dan dapat digunakan sebagai berikut:
